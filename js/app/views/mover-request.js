(function($){
	$(function(){
	  ...
	  var moveMapEl = $('#move-route-gmap'),

	      moveFromPlace = moveMapEl.attr('data-fromplace'),

	      moveToPlace = moveMapEl.attr('data-toplace'),

	      //console.log(moveToPlace);
	  	  moveMap = new google.maps.Map(moveMapEl.get(0), {
	      ...
	      moveMapGeocoder = new google.maps.Geocoder(),

	      moveMapDirectionDisplay = new google.maps.DirectionsRenderer({
	      	map: moveMap
	      }),

	      showMapError = function(message)
	      {
	      	  var messageContainer = moveMapEl.parent();
	      ...
	      getMoveRouteDirectionRequest = function(callback){
	      ...
	      showMoveRouteDirection = function(){
	      	 getMoveRouteDirectionRequest(function(request){
		      	 ...
		      	 moveMapDirectionsService.route(request, function(response, status) {
				    if (status == google.maps.DirectionsStatus.OK) {
				      ...
				      $('#route-distance .length .value').text(Math.round(routeDistance));
				      $('#distance').val(Math.round(routeDistance));
				    }
				    else
				    {
