$(document).ready(function(){
		...
		var PICKUP_KM_FARE = 1000/3;
		var CANTER_BASE_FARE = 2500;
		var CANTER_KM_FARE = 1400/3;
		var FH_BASE_FARE = 4500;
		var FH_KM_FARE = 2000/3;
		...
		var pickup_cost = PICKUP_BASE_FARE + distance*PICKUP_KM_FARE;
		$('#pickup_cost').text(Math.ceil(pickup_cost).toLocaleString());

		//initial canter calculation
		var canter_cost = CANTER_BASE_FARE + distance*CANTER_KM_FARE;	
		$('#canter_cost').text(Math.ceil(canter_cost).toLocaleString());

		//initial fh calculation
		var fh_cost = FH_BASE_FARE + distance*FH_KM_FARE;	
    	...
    	$('#interior_decorator').change(function(){
		...
		var write_quote = function() {
			if($('#pickup_radio').is(':checked')) {
		   		...
		   		$('#base_charge_cost').text(PICKUP_BASE_FARE.toLocaleString() + " /=");
		   		...
		   		if($('#pickup_loaders').val() > 0) {
		   			//unhide loaders label
		   			$('#loaders_label').removeClass('hidden');

		   			if($('#floor').val() === "ground") {
		   				var loaders_cost = HELPER_GROUND*$('#pickup_loaders').val();
		   				$('#loaders_cost').text(loaders_cost.toLocaleString() + " /=");;
		   			} else {
		   				var loaders_cost = HELPER_NOT_GROUND*$('#pickup_loaders').val();
		   				$('#loaders_cost').text(loaders_cost.toLocaleString() + " /=");
		   			}
		   	...
		   	if($('#canter_radio').is(':checked')) {
		   		...
		   		//populate base charge value
		   		$('#base_charge_cost').text(CANTER_BASE_FARE.toLocaleString() + " /=");
	   		...
	   		//populate base charge value
	   		$('#base_charge_cost').text(FH_BASE_FARE.toLocaleString() + " /=");
